...
export default class GSUtil {
	...
	*/
	...
	static position(placement = 'top', source, target, arrow) {
		...
		if (!(isStart || isEnd || isTop || isBottom)) {
		...
		if (arrow) {
			const arrowPosW = (source.offsetWidth / 2) - (arrow.offsetWidth / 2);
			const arrowPosH = (source.offsetHeight / 2) - (arrow.offsetHeight / 2);
			...
			if (isStart || isEnd) {
				...
				arrow.style.top = '0px';
				arrow.style.transform = `translate(0px, ${arrowPosH}px)`;
			} else {
				arrow.style.top = null;
				arrow.style.left = '0px';
				arrow.style.transform = `translate(${arrowPosW}px, 0px)`;
			}
		}

		let x = (rect.width / 2) + rect.left - (source.offsetWidth / 2);
		let y = rect.top + (rect.height / 2) - (source.offsetHeight / 2);
		if (isTop) {
			y = rect.top - source.offsetHeight;
		} else if (isBottom) {
			y = rect.top + rect.height;
		} else if (isStart) {
			x = rect.left - source.offsetWidth;
		} else if (isEnd) {
